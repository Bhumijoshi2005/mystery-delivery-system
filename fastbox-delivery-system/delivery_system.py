...
def assign_packages_to_agents(warehouses, agents, packages):
    ...
    for pkg in packages:
        ...
        if warehouse_id not in nearest_agent_for_warehouse:
            ...
            # Tie-breaking rule: if distances are equal, prefer the agent with
            # fewer currently assigned packages to balance the workload.
...
def simulate_deliveries(warehouses, agents, assignments):
    ...
    for agent_id, agent_packages in assignments.items():
        ...
        total_distance = 0.0
        remaining_packages = list(agent_packages)

        while remaining_packages:
            next_pkg = min(
                remaining_packages,
                key=lambda pkg: (
                    euclidean_distance(current_location, pkg["destination"]),
                    pkg["id"],
                )
            )
            remaining_packages.remove(next_pkg)

            warehouse_location = warehouses[next_pkg["warehouse"]]
            destination_location = next_pkg["destination"]

...
After the assignment, I simulate the route of each agent by choosing the
nearest remaining delivery destination from the agent's current position and
then traveling from the agent to the warehouse and from the warehouse to the
destination. The program then calculates the total distance, number of
delivered packages, and average distance per package. Finally, it saves the
result in `report.json`.
...
1. For each package, the agent travels from the current position to the
	warehouse and then from the warehouse to the package destination.

2. After each delivery, the agent selects the nearest remaining delivery
	destination from its current position and continues from there.

3. After the last delivery, the agent does not return to a warehouse because
	the report measures delivery travel only.

4. If two agents are the same distance from a warehouse, I assign the package
	to the agent with fewer packages currently assigned so the workload remains
	balanced.
